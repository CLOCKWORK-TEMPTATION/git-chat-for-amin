...
const TreeNodeComponent: React.FC<{ node: TreeNode; onFileClick: (file: FileNode) => void; depth: number }> = ({ node, onFileClick, depth }) => {
  ...
  const FolderIcon = () => (
    <svg className={`w-4 h-4 mr-2 text-tertiary/70 transition-transform ${isExpanded ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-border-subtle bg-base shrink-0">
          <div className="flex items-center gap-2">
            <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1 rounded-md text-text-secondary hover:bg-white/10 hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="px-3 py-3 border-b border-border-subtle shrink-0">
            <div className="relative group">
                <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <svg className="h-4 w-4 text-text-secondary group-focus-within:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Filters */}
        {extensions.length > 0 && (
            <div className="px-3 py-3 border-b border-border-subtle overflow-x-auto whitespace-nowrap custom-scrollbar shrink-0" dir="rtl">
                <button
        ...
        <div className="overflow-y-auto flex-1 p-3 dir-rtl custom-scrollbar">
             {tree.length === 0 ? (
                 <div className="flex flex-col items-center justify-center mt-10 text-text-secondary space-y-2">
                     <svg className="w-8 h-8 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
