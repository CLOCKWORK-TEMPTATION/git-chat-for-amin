    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-elevated-3 w-full max-w-md rounded-2xl border border-border-subtle shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-6 space-y-6">
            ...
            <div className={`flex items-center justify-between p-3 rounded-xl border transition-all ${settings.modelId === 'gemini-3-pro-preview' ? 'bg-base border-border-subtle' : 'bg-base/30 border-transparent opacity-50 cursor-not-allowed'}`}>
                <div className="flex items-center gap-3">
            ...
            <div className="space-y-2">
                <label className="text-sm font-medium text-text-secondary">رمز الوصول الشخصي (GitHub Token)</label>
                <input 
                    type="password"
            ...
            <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                    {AVAILABLE_MODELS.map(model => (
                        ...
                        >
                            <div className={`w-4 h-4 mt-0.5 rounded-full border flex items-center justify-center ${settings.modelId === model.id ? 'border-primary' : 'border-text-secondary'}`}>
                                {settings.modelId === model.id && <div className="w-2 h-2 rounded-full bg-primary"></div>}
                            </div>
                            <div>
                                <div className={`text-sm font-medium ${settings.modelId === model.id ? 'text-white' : 'text-text-secondary'}`}>{model.name}</div>
