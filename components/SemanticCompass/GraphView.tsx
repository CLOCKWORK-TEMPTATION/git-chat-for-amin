...
const GraphView: React.FC<GraphViewProps> = ({ data }) => {
    ...
    const getNodeColor = (type: NodeType) => {
        switch (type) {
            case 'FILE':
                return '#5A5296' // Primary
            case 'COMPONENT':
                return '#3DB89A' // Secondary
            case 'CONCEPT':
                return '#9A8EB3' // Tertiary
            case 'ISSUE':
                return '#C75D5D' // Error
            case 'DOC':
                return '#D4A853' // Warning
            case 'SLACK_THREAD':
                return '#ec4899'
            default:
                return '#64748b'
    ...
    return (
        <div className="flex h-full w-full bg-[#050508] relative overflow-hidden">
            ...
            <div className="absolute top-4 left-4 z-10 bg-elevated-2/80 backdrop-blur-md p-3 rounded-xl border border-border-subtle text-xs">
                ...
                <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#5A5296]"></span>
                        <span className="text-text-secondary">Files</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#3DB89A]"></span>
                        <span className="text-text-secondary">Components</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#9A8EB3]"></span>
                        <span className="text-text-secondary">Concepts</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#C75D5D] animate-pulse"></span>
                        <span className="text-text-secondary">Risk/Issues</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#D4A853]"></span>
                        <span className="text-text-secondary">Docs (Wiki)</span>
                    </div>
                </div>
            </div>
            ...
            <div
                className={`absolute top-0 right-0 h-full w-80 bg-elevated-2/95 backdrop-blur-xl border-l border-border-subtle shadow-2xl transform transition-transform duration-300 ease-in-out ${selectedNode ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto`}
            >
                {selectedNode && (
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-text-secondary leading-relaxed">
                                {selectedNode.metadata.description ||
                                    'No specific AI analysis available for this node.'}
                            </p>
                        </div>
                        ...
                        {selectedNode.metadata.riskLevel && selectedNode.metadata.riskLevel !== 'LOW' && (
                            <div className="p-4 bg-error/10 border border-error/30 rounded-lg space-y-2">
                                ...
                                <p className="text-xs text-red-300">
                                    This entity has been flagged with <strong>{selectedNode.metadata.riskLevel}</strong>{' '}
                                    severity. Review dependencies immediately.
                                </p>
                            </div>
                        )}
