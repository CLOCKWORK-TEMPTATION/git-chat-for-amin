...
import { Message, AppStatus, RepoInfo, FileNode, AppSettings, YoutubeInfo, FileContent, KnowledgeGraph, Challenge, UserProfile, SubmissionResult } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Auto-scroll to bottom of chat if not searching
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      } else if ((trimmed.includes('stackoverflow.com') || trimmed.includes('stackexchange.com')) && inputMode !== 'stackoverflow') {
          setInputMode('stackoverflow');
      } else if (trimmed.startsWith('http') && !trimmed.includes('github.com') && !trimmed.includes('youtube.com') && !trimmed.includes('stackoverflow.com') && inputMode !== 'url') {
          setInputMode('url');
      }
  };
    ...
    <div className="flex flex-col h-screen bg-base text-text-secondary font-sans overflow-hidden" dir="rtl">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-border-subtle bg-surface/80 backdrop-blur-xl z-20 sticky top-0">
        <div className="flex items-center gap-3 overflow-hidden">
          <div className={`p-2.5 rounded-xl shadow-lg flex-shrink-0 bg-primary/20 text-primary border border-primary/30`}>
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-aurora tracking-tight truncate">GitChat AI</h1>
             {status === AppStatus.READY && currentTitle ? (
                ...
                <p className="text-[10px] text-tertiary/70 uppercase tracking-widest font-semibold hidden md:block">Code & Video Analyst</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
             ...
             {status === AppStatus.READY && (
                 <>
                    ...
                    <div className={`flex items-center transition-all duration-300 ${isChatSearchOpen ? 'w-48 md:w-64 bg-surface rounded-lg border border-border-subtle' : 'w-8'}`}>
                        {isChatSearchOpen ? (
                             <>
                                <input 
                                    autoFocus
                                ...
                                />
                                <button onClick={() => { setIsChatSearchOpen(false); setChatSearchQuery(''); }} className="p-2 text-text-secondary hover:text-white">
                                    <CloseIcon />
                                </button>
                             </>
                        ) : (
                            <button onClick={() => setIsChatSearchOpen(true)} className="p-1.5 text-text-secondary hover:text-white transition-colors" title="بحث">
                                <SearchIcon />
                            </button>
                        )}
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-2 text-xs text-text-secondary bg-surface py-2 px-4 rounded-full border border-border-subtle backdrop-blur-md max-w-[200px]">
                        ...
                        <span className="text-white font-medium truncate" dir="ltr">{currentTitle}</span>
                    </div>
                    )}
      ...
      {/* Main Content */}
      <main className={`flex-1 flex flex-col relative max-w-5xl mx-auto w-full transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1)] ${isSidebarOpen ? 'mr-0 md:mr-80' : ''}`}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-fade-in relative z-10">
            ...
            <div className="max-w-md w-full space-y-8 relative">
              ...
              <div className="flex flex-wrap p-1 bg-surface backdrop-blur-sm rounded-xl border border-border-subtle w-fit mx-auto overflow-hidden justify-center gap-1">
                  ...
                  >
                       <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.986 21.865v-6.404h2.134V24H1.844v-8.539h2.13v6.404h15.012zM6.111 19.79h12.668v-2.125H6.111v2.125zm.94-5.43l12.151 2.591.473-2.06L7.522 12.3l-.472 2.06zm1.733-5.085l10.95 6.435 1.066-1.839-10.95-6.435-1.066 1.839zm3.118-4.711l8.635 9.323 1.55-1.464-8.635-9.322-1.55 1.463zM14.507.085l-1.91 1.413 5.968 12.307 1.91-1.413L14.507.085z"/></svg>
                ...
                <div className="relative group">
                  {inputMode === 'local' ? (
                    ...
                    <input
                        ...
                        placeholder={
                            inputMode === 'github' ? "https://github.com/owner/repo" : 
                            inputMode === 'url' ? "https://docs.example.com/..." :
                            inputMode === 'stackoverflow' ? "https://stackoverflow.com/questions/..." :
                            inputMode === 'package' ? "react, pandas, lodash..." :
                ...
                {(status !== AppStatus.IDLE && status !== AppStatus.ERROR) ? (
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            <span className="text-sm">
                              {status === AppStatus.FETCHING_REPO ? 'جاري الاتصال...' : 
                               status === AppStatus.ANALYZING ? 'جلب الملفات...' : 
                               status === AppStatus.BUILDING_GRAPH ? 'بناء البوصلة الدلالية...' : 
                               'فهرسة المحتوى (RAG)...'}
                            </span>
                        </button>
                        <button
                    ...
                    inputMode !== 'local' && (
                        ...
                        >
                            ...
                            <svg className="w-5 h-5 transform rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                        </button>
                    )
                )}
              </form>
              ...
              {status === AppStatus.ERROR && (
                <div className="p-4 bg-error/10 border border-error/20 rounded-xl flex items-center gap-3 text-red-200 text-sm backdrop-blur-sm animate-shake text-right">
                   <svg className="w-5 h-5 text-error flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                  <span>{statusMessage}</span>
                </div>
              )}
              
              {/* Same status text... */}
              {(status === AppStatus.ANALYZING || status === AppStatus.INDEXING || status === AppStatus.FETCHING_REPO || status === AppStatus.BUILDING_GRAPH) && (
                <div className="text-sm text-tertiary animate-pulse font-mono bg-tertiary/5 p-3 rounded-lg border border-tertiary/10" dir="ltr">
                  &gt; {statusMessage}
                </div>
              )}
            </div>
             ...
             {viewMode === 'compass' && knowledgeGraph ? (
                 ...
                 <>
                        ...
                        <div className="px-4 py-4 border-b border-border-subtle bg-black/20 shrink-0">
                            <div className="max-w-2xl mx-auto flex flex-col md:flex-row gap-4">
                                ...
                                <div className="flex flex-col justify-center gap-3">
                                    <h3 className="text-sm font-medium text-white line-clamp-2">{youtubeInfo.title}</h3>
                                    <button 
                    ...
                    <div className="max-w-3xl mx-auto">
                        <form onSubmit={handleSendMessage} className="relative flex flex-col bg-elevated-2/90 backdrop-blur-xl border border-border-subtle shadow-2xl rounded-2xl ring-1 ring-white/5 group focus-within:ring-primary/50 transition-all">
                                ...
                                <div className="px-4 py-3 border-b border-border-subtle flex items-center justify-between animate-fade-in bg-base/50 rounded-t-2xl">
                                    <div className="flex items-center gap-3">
                                        <div className="h-12 w-12 rounded-lg bg-black/50 overflow-hidden border border-border-subtle">
                                            <img src={`data:${selectedImage.mimeType};base64,${selectedImage.data}`} alt="Selected" className="h-full w-full object-cover" />
                                        </div>
                            ...
                            <div className="flex items-center w-full">
                                <input
                                    ...
                                    placeholder={inputMode === 'youtube' ? "اسأل عن محتوى الفيديو..." : "اكتب سؤالك هنا..."}
        ...
        {status === AppStatus.READY && (
            <SkillTreeWidget profile={userProfile} />
        )}
